...
app = Flask(__name__)
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///generated_files.db'
...
def index():
    ...
    if request.method == 'POST':
        ...
        copywriting_texts = request.form.getlist('copywriting_text')
        ...
        <div id="container">
            <textarea id="textToCopy" rows="10" cols="50">
        {copywriting_text}
            </textarea>
        ...
        for index, data in enumerate(media_data):
            ...
            # Save to database
            new_file = GeneratedFile(filename=output_file_name, content=html_content)
            db.session.add(new_file)
            db.session.commit()

        for index, copywriting_text in enumerate(copywriting_texts):
            file_name = f"{main_title}_copywriting_{index + 1}.html"
            number = index + 1

            html_content = html_template_copywriting.format(copywriting_text=copywriting_text)

            output_file_name = f"{output_dir}/{main_title}_copywriting_{number}.html"
            with open(output_file_name, 'w', encoding='utf-8') as file:
                file.write(html_content)
            generated_html_files.append({"file": output_file_name, "content": html_content})
            print(f"Generated HTML file for copywriting: {output_file_name}")
...
@app.route('/history')
def history():
    history = GeneratedFile.query.all()
